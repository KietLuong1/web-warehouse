...
export const useDashboardData = (dateRange = null, refreshInterval = null) => {
    const { data: dashboardData, loading: dashboardLoading, error: dashboardError } = useDashboard(dateRange, refreshInterval);
    ...
    const transactionAnalytics = useMemo(() => {
        ...
        for (let i = 0; i < 6; i++) {
            const monthlyQuantity = Math.floor(dashboardData.transactionMetrics.transactionCount * (0.15 + Math.random() * 0.1));
