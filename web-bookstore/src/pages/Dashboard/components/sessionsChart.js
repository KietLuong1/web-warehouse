...
function AreaGradient({ color, id }) {
    return (_jsx("defs", { children: _jsxs("linearGradient", { id: id, x1: '50%', y1: '0%', x2: '50%', y2: '100%', children: [_jsx("stop", { offset: '0%', stopColor: color, stopOpacity: 0.5 }), _jsx("stop", { offset: '100%', stopColor: color, stopOpacity: 0 })] }) }));
}
...
export default function SessionsChart() {
    ...
    const totalTransactions = useMemo(() => {
        ...
        return {
            total: transactionAnalytics.dailyTransactions.reduce((sum, day) => sum + day.totalQuantity, 0),
    ...
    if (error || !chartData) {
        return (_jsxs(Card, { sx: { width: '100%', height: 350 }, children: [' ', _jsx(CardContent, { children: _jsx(Typography, { color: 'error', children: "Failed to load transaction data" }) })] }));
    }
    return (_jsx(Card, { sx: { width: '100%' }, children: _jsxs(CardContent, { children: [' ', _jsx(Typography, { component: 'h2', variant: 'subtitle2', gutterBottom: true, children: "Warehouse Transactions" }), _jsxs(Stack, { sx: { justifyContent: 'space-between' }, children: [_jsxs(Stack, { direction: 'row', sx: {
                                alignContent: { xs: 'center', sm: 'flex-start' },
                                alignItems: 'center',
                                gap: 1
                            }, children: [_jsx(Typography, { variant: 'h4', component: 'p', children: totalTransactions.total.toLocaleString() }), _jsx(Chip, { size: 'small', color: totalTransactions.change >= 0 ? 'success' : 'error', label: `${totalTransactions.change >= 0 ? '+' : ''}${totalTransactions.change}%` })] }), _jsx(Typography, { variant: 'caption', sx: { color: 'text.secondary' }, children: "Transaction volume for the last 30 days" })] }), _jsxs(LineChart, { colors: colorPalette, xAxis: [
                        {
                            scaleType: 'point',
                            data: chartData.days,
                    ...
                    ], height: 250, margin: { left: 50, right: 20, top: 20, bottom: 20 }, grid: { horizontal: true }, sx: {
                    ...
                    }, children: [_jsx(AreaGradient, { color: theme.palette.primary.dark, id: 'transfer' }), _jsx(AreaGradient, { color: theme.palette.primary.main, id: 'outbound' }), _jsx(AreaGradient, { color: theme.palette.primary.light, id: 'inbound' })] })] }) }));
}
